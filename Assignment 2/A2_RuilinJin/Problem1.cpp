...

    {
        ...
        q.submit([&](handler& h) {
            ...
            h.parallel_for<MatrixMulKernel>(
                range<2>{N / 2, N / 2}, [=](id<2> idx) {
                    int halfN = N / 2;
                    int global_x = idx[0];
                    int global_y = idx[1];

                    for (int k = 0; k < N; k++) {
                        // Compute for all 4 quadrants
                        accC[global_y * N + global_x] += accA[global_y * N + k] * accB[k * N + global_x];
                        accC[(global_y + halfN) * N + global_x] += accA[(global_y + halfN) * N + k] * accB[k * N + global_x];
                        accC[global_y * N + global_x + halfN] += accA[global_y * N + k] * accB[k * N + global_x + halfN];
                        accC[(global_y + halfN) * N + global_x + halfN] += accA[(global_y + halfN) * N + k] * accB[k * N + global_x + halfN];
                    }
                });
        });
    }
